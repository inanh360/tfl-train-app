...
**A real bug and decision log.** Every non trivial bug found during development, why it happened, and how it was fixed is written up in BUGFIXES.md in this repository. It is the most honest record of the actual engineering work behind this project.
...
**Infrastructure**
Docker and Docker Compose for every service. Caddy as a reverse proxy with automatic HTTPS. A Linux server on Hetzner Cloud, hardened with a proper firewall and SSH key only access. GitHub Actions for automatic deployment on every push.

**External data**
Transport for London's own public Unified API, used for live status, journey planning, and arrival predictions.

## Local development

Clone the repository, copy `.env.example` to `.env` and fill in real values, then run:
