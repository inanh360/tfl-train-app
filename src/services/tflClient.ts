...
// showing every operator that happens to share the building.
export async function getStationDetail(stationId: string): Promise<StationDetail> {
  ...
  const res = await fetch(url.toString());
  if (!res.ok) {
    throw new Error(`TfL station detail request failed: ${res.status} ${res.statusText}`);
  }

  const detail = (await res.json()) as {
    id: string;
    commonName: string;
    lines?: { id: string; name: string; modeName?: string }[];
  };

  const lines = (detail.lines ?? []).filter(
    (l) => l.modeName && (TRAIN_MODES as readonly string[]).includes(l.modeName)
  );

  return { id: detail.id, name: detail.commonName, lines: lines.map((l) => ({ id: l.id, name: l.name })) };
}
